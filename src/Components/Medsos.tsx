...
const Medsos: React.FC<MedsosProps> = ({ isOpen, isDarkMode }) => {
  return (
  <div id='medsos' className={`container mx-auto   transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`} style={{marginTop: '',}}>
      ...
      <div className="flex flex-wrap justify-center   p-4 md:flex-nowrap">
        {socialMediaLinks.map((media) => (
          <div key={media.name} className={` ${isDarkMode ? 'bg-gray-800' : 'bg-white'} card-icon shadow-lg rounded-lg p-3 w-full md:w-48 text-center mx-2 ${media.hover} ${media.textHover}`}>
